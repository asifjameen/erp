...
$(document).ready(function () {
	...
	// 	type: "post",
	// 	url: "../code.php",
	// 	data: {'action':'invoice_last_id'},
	// 	//dataType: "dataType",
	// 	success: function (response) {
	// 		var json=JSON.parse(response)
	// 	//alert("json['invoice']");	
	// 	$('#invoice_id').val(parseInt(json['invoice'])+1);
	// 	}
	// });
	...
	$('#invoice_type').change(function () {
		...
		if (invoiceType == 'Receipt') {
			...
			$('input.remove_vat').prop("checked", true);
			$('.tax').hide();
			$('.invoice_cgst').hide();
			$('#invoice_sgst').hide();
			...
			$.ajax({
				type: "post",
				url: "code.php",
				data: { 'action': 'receipt_last_id' },
				//dataType: "dataType",
				success: function (response) {
					if (response == 0) {
						$('#invoice_id').val(parseInt(response) + 1);
					}
					else {
						var json = JSON.parse(response)
		...
		if (invoiceType == 'quote') {
			...
			$('input.remove_vat').prop("checked", false);
			$('.tax').show();
			$('#invoice_cgst').show();
			$('.invoice_sgst').show();
		}
	...
	$('#invoice_status').change(function () {
		var invoicestatus = $("#invoice_status option:selected").text();
		$(".invoice_type").text(invoicestatus);
		//
		if (invoicestatus == 'Paid') {
			//$('.invoice_due_date"').hide();
			$('.due_date').hide();
			//alert(invoicestatus);
		}
		else {
	...
	$(document).on('click', ".item-select", function (e) {
		...
		$('#insert').modal('show').one('click', '#selected', function (e) {

			var itemText = $('#insert').find("option:selected").text();
			var itemValue = $('#insert').find("option:selected").val();
			var itemid = $('#insert').find("option:selected").attr("id");
			var itemqty = $('#insert').find("option:selected").attr("name");
			var pur_price = $('#insert').find("option:selected").attr("pur_price");
			console.log(itemid);
			console.log(pur_price);
			$(product).closest('tr').find('.pid').val(itemid);
			$(product).closest('tr').find('.purchase_price').val(pur_price);
			$(product).closest('tr').find('.invoice_product_avl_qty').val(itemqty);
			$(product).closest('tr').find('.invoice_product').val(itemText);
			$(product).closest('tr').find('.invoice_product_price').val(itemValue);
			$(product).closest('tr').find('.subbb').val(itemValue);
	...
	$("#invoice_create").click(function (e) {
		e.preventDefault();
		frm = $('#create_invoice_frm').serialize();
		$.ajax({
			type: "method",
			url: "code1.php",
			data: frm,
	...
	$(document).on('click', ".sss", function (e) {
		...
		//var product = $(this);
		//	$(product).closest('tr').find('.ss').click()
		//alert("J");
	...
	var cloned = $('#invoice_table tr:last').clone();
	$('#invoice_table').on('click', ".add-row", function (e) {
		e.preventDefault();
		cloned.clone().appendTo('#invoice_table');
		calculateTotal();
	});
	...
	// 			$('#invoice_sgst').val((((vat / 100) / 2) * finalTotal).toFixed(2));
	// 			$('#invoice_cgst').val((((vat / 100) / 2) * finalTotal).toFixed(2));
	...
	// 		$('.invoice_cgst').text("0.00");
	// 		$('#invoice_cgst').val("0.00");
	// 		$('.invoice_sgst').text("0.00");
	// 		$('#invoice_sgst').val("0.00");
	// 		$('.invoice-total').text((finalTotal).toFixed(2));
	// 		$('#invoice_total').val((finalTotal).toFixed(2));
	// 	}
