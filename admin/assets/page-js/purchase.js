...
$(document).ready(function () {
    ...
    $(document).on('change', '#p_product_name', function (event) {
        ...
        $.post('controller/product.php', { action: 'readone', 'id': $p_product_id }, function (data) {
            data_json = JSON.parse(data)
            data = data_json.data;
            console.log(data[0].productQuantity);
            $('#prev_avl_stk_qty').val(data[0].productQuantity);
            $('#prev_pur_price').val(data[0].productPurchasePrice);
            $('#prev_sales_price').val(data[0].productSalesPrice);
        });
    });
    ...
    $(document).on('keyup', '#p_p_price', function (event) {
        event.preventDefault();
        /* Act on the event */
        var purchas_quantity = $('#p_pn_quantity').val();
        var purchas_price = $('#p_p_price').val();
    ...
    $(document).on('click', '#addByproductBtn', function (event) {
        ...
        var data = {
            suppliar_id: $('#p_supplier').val(),
            suppliar_name: $('#p_supplier option:selected').text(),

            purchase_date: $('#current_date').val(),
            
            product_id: $('#p_product_name').val(),
            new_purchase_quantity: $('#p_pn_quantity').val(),
            new_purchase_price: $('#p_p_price').val(),
            new_sell_price: $('#p_p_sell_price').val(),
            payMethode: $('#p_payMethode').val(),
            product_name: $('#p_product_name option:selected').text(),
            pre_quantity: $('#prev_avl_stk_qty').val(),
            subtotal: $('#p_subtotal').val()
        };
        ...
        $.ajax({
            url: 'controller/purchase.php',
            method: 'POST',
            data: { 'data': serializedData, 'action': 'purchase' },
            ...
            success: function (response) {
                ...
                if (response.status ==='success') {
                    ...
                    swal({
                        ...
                        if (willReload) {
                            $('#addByproductForm').find('input').val('');
                            $('#p_product_name').val(null).trigger('change'); // Reset Select2
                        }
                    });
